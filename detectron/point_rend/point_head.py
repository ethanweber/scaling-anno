...
#         mask_logits, gt_mask_logits.to(dtype=torch.float32), reduction="mean"
#     )
#     return point_loss, mask_accuracy


def roi_mask_point_min_loss(diversity_mask_logits, instances, points_coord, prob_random=0.05):
    """
    Compute the point-based loss for instance segmentation mask predictions.

    Args:
        mask_logits (List): A List of tensors of shape (R, C, P) or (R, 1, P) for class-specific or
            class-agnostic, where R is the total number of predicted masks in all images, C is the
            number of foreground classes, and P is the number of points sampled for each mask.
            The values are logits.
        instances (list[Instances]): A list of N Instances, where N is the number of images
    ...
    # iterate over the logits, compute the loss, and return only the loss with the minumum error
    # with some probability, train a random output

    # ramp up to prob_random
    iteration = get_event_storage()._iter
    # TODO(ethan): move this out to a config
    WARMUP_ITERATIONS = 1000.0
    # print(iteration)
    prob = prob_random + (1.0 - prob_random) * \
        (1.0 - min(iteration / WARMUP_ITERATIONS, 1.0))
    get_event_storage().put_scalar("point_rend/prob_random", prob)
    random_index = False
    if random.uniform(0, 1) < prob:
        random_index = True
        # random_index = random.randint(0, len(diversity_mask_logits) - 1)

    if random_index:
        # don't plot when random
        # mask_logits = diversity_mask_logits[random_index]
        min_loss = 0.0
        for i in range(len(diversity_mask_logits)):
            mask_logits = diversity_mask_logits[i]
            point_loss, _ = roi_mask_point_loss(
                mask_logits, instances, points_coord)
            min_loss += point_loss
        min_loss /= len(diversity_mask_logits)
    else:
        min_index = None
        selected_accuracy = None
        for i in range(len(diversity_mask_logits)):
            mask_logits = diversity_mask_logits[i]
            point_loss, accuracy = roi_mask_point_loss(
                mask_logits, instances, points_coord)
            if min_index is None or point_loss < min_loss:
                min_index = i
                min_loss = point_loss
                selected_accuracy = accuracy

        get_event_storage().put_scalar("point_rend/mask_index", min_index)
        get_event_storage().put_scalar("point_rend/accuracy", selected_accuracy)

    return min_loss


def roi_mask_point_diversity_loss(diversity_mask_logits, coeff=0.01):
    """
    Compute the point-based diversity loss for instance segmentation mask predictions.

    Args:
        diversity_mask_logits (List): A List with of tensors of shape (R, C, P) or (R, 1, P) for class-specific or
            class-agnostic, where R is the total number of predicted masks in all images, C is the
            number of foreground classes, and P is the number of points sampled for each mask.
            The values are logits.
    Returns:
    ...
    count = 0
    for i in range(len(diversity_mask_logits)):
        for j in range(i+1, len(diversity_mask_logits)):
            mask_logits_i = diversity_mask_logits[i].sigmoid()
            mask_logits_j = diversity_mask_logits[j].sigmoid()
            diversity_loss += (1.0 - F.mse_loss(mask_logits_i, mask_logits_j))
            count += 1
    diversity_loss /= count
    diversity_loss *= coeff
    get_event_storage().put_scalar("point_rend/diversity", diversity_loss)
...
class StandardPointHead(nn.Module):
    ...
    def __init__(self, cfg, input_shape: ShapeSpec):
        ...
        for k in range(num_fc):
            fc = nn.Conv1d(fc_dim_in, fc_dim, kernel_size=1,
